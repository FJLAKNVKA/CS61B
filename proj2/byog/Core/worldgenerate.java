...
public class worldgenerate {
    ...
    private Point wall[][] = new Point[4][WIDTH+HEIGHT];  //按照上左下右的排列
    ...
    private boolean maze_load[][] = new boolean[WIDTH+1][HEIGHT+1];

    public worldgenerate(long Seed)
    {
        ...
        while (true)
        {
            ...
            if(world[x][y] == Tileset.WALL)
            {
                ...
                for(int i=0;i<4;i++)
                {
                    ...
                    if(world[a][b]==Tileset.FLOOR)out = true;
                }
                if(out)break;
            }
        }
    ...
    public TETile[][] Theworld()
    {
        return world;
    }

    private boolean check(int starx,int stary,int len,int wid) //检查房子是否重叠和是否越界
    {
    ...
    private void bulid(int starx,int stary,int len,int wid)
    {
        //rst用来标记墙的
    ...
    private void add_list(int x,int y,int idx)    //传入的是一个floor周围的墙
    {
        if(num>0) remove(idx);
        else return;
    ...
    private void remove(int idx)  //用掉的点要从队伍中去除掉：和最后一个点做交换，然后让队伍长度减一即可
    {
        if(idx==-1)return;
        int x = grid[num].x;
        int y = grid[num].y;
    ...
    private void dfs_grass(int x,int y)
    {
        for(int i=0;i<4;i++)
        {
    ...
    private void find_wall(int x,int y)  //传入贴着墙的房间块
    {
        for(int i=0;i<4;i++)
        {
            int a = x+dx[i],b = y+dy[i];
    ...
    private boolean bodercheck(int x,int y)
    {
        return (x>=0&&x<WIDTH&&y>=0&&y<HEIGHT);
    }

    private boolean checkground(int a,int b)
    {
        for(int i=0;i<4;i++)
        {
            int x = a+dx[i],y = b+dy[i];
    ...
    private boolean search(int x,int y,Random random)
    {
